...
class CardTests(unittest.TestCase):
	...
	def size(self):
		size = deck()
		self.assertEqual(len(size.cards), 52)

	def test_deckpopInstance(self):
		pop = Deck()
		card = pop.pop_card()
...
# Test that if you invoke the __str__ method of a card instance that is created with suit 1, rank 1, it returns ace of clubs
	def test_myTestInstance1(self):
		Ace = Card(suit = 1, rank = 1)
		self.assertEqual(str(Ace),"Ace of Clubs")

	
# Test that if you invoke the __str__ method of a card instance that is created with suit 2, rank 12, it returns Queen of Hearts
	def test_myTestInstance2(self):
		Queen = Card(suit = 2, rank = 12)
		self.assertEqual(str(Queen),"Queen of Hearts")
